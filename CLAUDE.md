...
- **AI API**: Pluggable provider layer (Gemini/Anthropic/Ollama/OpenAI adapters); Google Gemini (`gemini-2.5-flash`) is the default
...
Generate AI persona via database seeding (first season) and automatic seasonal rotation:
...
### AI Integration
- Use the AI provider layer (Gemini by default) for:
  - Persona generation
  - Message generation
  - Timing decisions
...
- AI-driven usage decisions (via the AI provider layer)
...
- `farewell_sent_at`: Durable record that the deactivation farewell was delivered (set with the first farewell fragment)
...
**AI Provider Layer** (`app/services/` — pluggable adapter architecture)
- `AiProviders`: ENV/database-driven factory (`build`, `build_router`)
- `AiProviders::Base`: Adapter interface (`complete(model:, system:, prompt:, max_tokens:)` → `AiResponse`; adapters never raise — nil text signals failure)
- `AiProviders::GeminiAdapter` / `AnthropicAdapter` / `OllamaAdapter` / `OpenaiAdapter`: Provider adapters
- `AiModelRouter`: Maps application task symbols (`TASK_TIERS`) to model tiers; application logic names tasks, never providers or models
- `AiResponse`: Uniform response struct (text/model/provider/task)
...
- `PeriodicTasksJob`: Hourly maintenance and load-bearing recovery nets (farewell resend, decision-chain revival, unattached-blob purge)
- `SeasonRotationJob`: Handle season rotation
- `SeasonDeactivationReminderJob`: Send rotation warnings
- `SeasonFarewellJob`: Deliver deactivation farewells via the normal message flow, with durable delivery tracking
- `PushNotificationDeliveryJob`: Deliver web push notifications
...
- `DeviceLinksController`: Short-lived single-use codes for carrying a device identity into a new install
- `DirectUploadsController`: ActiveStorage direct uploads

**ActionCable Channels** (`app/channels/`)
- `ConversationChannel`: Broadcast-only stream for real-time events (messages, typing, read receipts) plus the subscription-presence lifecycle; client-to-server traffic travels over REST
...
- `useProfileEditor`: Shared profile-editing state (picture selection/validation/preview, save)
- `useClosingTransition`: Exit-animation timing for closing overlays and banners
...
- `Profile/ProfileSheet`: Profile viewing/editing
- `Onboarding/OnboardingFlow`: First-run onboarding wizard
- `Common/PromptBanner`: Shared dismissible prompt banner (used by both PWA prompts)
...
- `deviceLink.ts`: Device-link code validation and identity adoption (claim → persist → reload)
...
- Backend: `AI_PROVIDER` (gemini | anthropic | ollama | openai; defaults to 'gemini', overridden by a database AiProvider record), `GEMINI_API_KEY` (or configure via database), `GEMINI_MODEL` (defaults to 'gemini-2.5-flash'; or `GEMINI_ADVANCED_MODEL`/`GEMINI_STANDARD_MODEL` per routing tier), provider-specific keys for the alternatives (`ANTHROPIC_API_KEY`, `OLLAMA_HOST`, `OPENAI_API_KEY`/`OPENAI_BASE_URL`/`OPENAI_*_MODEL`), `REDIS_URL`, `DATABASE_URL`, `VAPID_PRIVATE_KEY`, `VAPID_PUBLIC_KEY`, `VAPID_SUBJECT` (contact mailto: for Web Push, required for push delivery), `APP_VERSION`, `BACKEND_URL` (public URL of the backend, used for attachment/notification URLs; defaults to 'http://localhost:3001'), `FRONTEND_URL` (production CORS allowed origin; defaults to 'http://localhost:3000')
- Frontend: `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_WS_URL`, `NEXT_PUBLIC_VAPID_PUBLIC_KEY`

## Implementation Status

### Completed Features ✅
- [x] **AI Provider Abstraction**: AiProviders adapter modules (Gemini/Anthropic/Ollama/OpenAI) behind a uniform `complete` interface, with AiModelRouter mapping task symbols to model tiers — application logic never references a provider or model
- [x] **Database-backed AI Configuration**: AiProvider model stores provider type and API key (encrypted); overrides ENV selection
- [x] **Provider-agnostic AI Services**: All AI services call the router with task symbols and a system/prompt split
...
- [x] **PWA Install Prompt**: beforeinstallprompt captured once per page load in a page-global store (so consumers mounting later still see it) and exposed as a single `installMethod` discriminant; on iOS Safari (which never fires the event) the banner and onboarding step show share-sheet instructions instead — in-app webviews and third-party iOS browsers, where that install path doesn't exist, get no install UI
- [x] **Profile Picture Upload**: ActiveStorage with direct upload pattern, file upload UI in onboarding and profile sheet
- [x] **Device Linking**: Redis-backed 6-char single-use codes (10-min TTL) let a new install (e.g. iOS home-screen PWA) adopt an existing device_id — issued from the profile sheet's DeviceLinkCard, claimed from onboarding's '코드로 이어가기' step or from that card while the device's identity is still throwaway (no user-sent messages); claiming self-cleans the abandoned pre-claim user row
- [x] **Ambient Prompt Sequencing**: NotificationPrompt and InstallPrompt never stack — install waits for a later session once the notification ask is settled (granted/denied/dismissed/unsupported), tracked via a sessionStorage defer flag since App Router remounts the prompts per navigation; on iOS Safari push is unsupported outside the installed app, so install correctly leads
...
- Anthropic/Ollama/OpenAI adapters are untested against live endpoints (Gemini is the exercised default)
- iOS install identity split (mitigated): home-screen PWAs on iOS get an isolated storage partition, so installing creates a fresh device_id (platform-inherent; no client-side handoff exists). The device-linking flow closes this — the original device issues a short-lived single-use code (profile sheet), and the new install claims it during onboarding — or later from the profile sheet while its identity is still throwaway — to adopt the existing device_id (the abandoned pre-claim user row is self-cleaned). Users who skip the flow still start fresh.
