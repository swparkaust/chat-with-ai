...
export default function ChatInput({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Visible feedback is load-bearing: vibrate() no-ops on iOS Safari
      // and most desktops
      setSendError(
        err instanceof ApiError ? err.getUserMessage() : "메시지 전송에 실패했습니다"
      );
  ...
  return (
    ...
    >
      {sendError && (
        <p className="px-1 pb-1.5 text-xs text-primary-red">{sendError}</p>
      )}
