module Ai
  class PersonaGenerator
    ...
    def generate(prompt)
      ...
      response = @router.call(task: :persona_generation, system: "You are a persona generator.", prompt: generation_prompt)

      validate_persona_data(Ai::JsonParsing.parse_object(response.text))
    end
    ...
    def build_generation_prompt(prompt, current_datetime, weather)
      <<~PROMPT
        Create a realistic Korean person based on this description: "#{prompt}"
    ...
    def validate_persona_data(data)
      # Single guard for blank text, unparseable JSON, and an empty parsed
      # object (all reach here as a blank hash); GenerationError engages the
      # bounded retry policies
      if data.blank?
        raise Ai::GenerationError, "페르소나 생성 실패: AI 응답이 비어있거나 파싱할 수 없습니다"
      end

      # Names are validated here, not at activation: an unnamed persona
      # would pass generation, then fail the active-season name validation
      # AFTER the expensive tool initialization — leaking a husk per attempt
      if data['first_name'].blank? || data['last_name'].blank?
        raise Ai::GenerationError, "페르소나 생성 실패: 이름이 누락되었습니다"
      end
    ...
    def validate_list_fields(data)
      list_fields = Ai::PersonaFields.array_field_names + %w[tags]
