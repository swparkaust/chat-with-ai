module Ai
  class SystemContextBuilder
    # track_recall: recall_count increments only where a memory demonstrably
    # shapes generated output — message generation passes true; decision
    # polls, timing, reevaluation, and evolution stay on the false default
    # so the memory-pruning signal reflects real usage, not access frequency
    def initialize(persona_state, conversation = nil, track_recall: false)
    ...
    def get_relevant_memories
      ...
      if @track_recall && memories.any?
        PersonaMemory.where(id: memories.map(&:id))
                     .update_all(['recall_count = recall_count + 1, last_recalled_at = ?', Time.current])
      end
    ...
    def get_tool_context
      Persona::Tools::ToolManager.new(@season).get_all_context
    end
    ...
    def build_context_string(current_datetime, weather, memories_str, tool_context)
      ...
      # Section order is deliberately most-stable-first: providers with
      # implicit prefix caching discount identical request prefixes, and
      # these persona sections dominate every prompt the app sends. The
      # volatile lines (datetime, weather) live near the end so they don't
      # break the shared prefix at token ~20.
      <<~CONTEXT
